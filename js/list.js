...
// ==================== RENDER BOOKINGS LIST (ASYNC + Loading) ====================
window.renderBookingsList = async function () {
  try {
    ...
    // Filter pencarian (tambahkan eventLocation)
    if (search) {
      ...
      bookings = bookings.filter(
        (b) =>
          ...
          (b.eventLocation || "").toLowerCase().includes(search),
      );
      console.log(
        `🔍 Search filter: ${beforeFilter} → ${bookings.length} results`,
      );
    }
    ...
    if (filter !== "all") {
      ...
      console.log(
        `🎯 Status filter (${filter}): ${beforeFilter} → ${bookings.length} results`,
      );
    }
      ...
      tbody.innerHTML = bookings
        .map(
          ...
          <tr class="border-b border-gray-50 hover:bg-gray-50 transition-all">
              ...
              <td class="px-4 lg:px-6 py-4 text-sm text-gray-500 hidden md:table-cell max-w-[120px] truncate" title="${escapeHtml(b.eventLocation) || "-"}">
                  ${escapeHtml(b.eventLocation) || "-"}
              </td>
              ...
              <td class="px-4 lg:px-6 py-4">
                  <div class="flex items-center gap-1">
                      ...
                      ${
                        (b.status || '').toLowerCase().trim() === "approved"
                          ? `
    ...
    setTimeout(() => {
      ...
      // ✅ Hide loading after QR codes rendered
      hideLoading();
    }, 100);
...
// ==================== MARK AS RETURNED (ASYNC - No Equipment Logic) ====================
window.markReturned = async function (id) {
  ...
  try {
    ...
    // Save updates (no equipment logic needed)
...
// ==================== SHOW DETAIL MODAL (ASYNC - With eventLocation) ====================
    ...
    const content = `
        ...
        <div class="p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between text-sm"><span class="text-gray-500">Tempat Kegiatan</span><span class="font-medium text-gray-800 text-right max-w-[60%]">${escapeHtml(booking.eventLocation) || "-"}</span></div>
