/**
 * ========================================
 * PRINT.JS - Print & QR Code Functions
 * ========================================
 */

// ==================== PRINT BOOKING WITH QR CODE (Updated - eventLocation) ====================
window.printBooking = async function (id) {
  try {
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .qr-wrap { text-align: center; margin: 15px 0; padding: 10px; background: #f5f5f5; border-radius: 8px; }
        ...
        <body>
            <div class="header">
                ...
                <p>Sound System Management Pro</p>
            </div>
            <div class="qr-wrap">
                <div id="printQRCode" class="qr-code-print"></div>
            </div>
            <div style="text-align:center; margin-bottom:15px;">
                <span style="font-family:monospace; font-size:14px; font-weight:bold; color:#4f46e5;">${booking.id}</span>
            </div>
            <div class="info">
                ...
                <div class="info-row"><span class="label">Tempat Kegiatan</span><span class="value">${escapeHtml(booking.eventLocation) || "-"}</span></div>
    ...
    showToast("Gagal cetak: " + (e.message || "Unknown error"), "error");
  }
};

// ==================== SHOW BOOKING RECEIPT WITH QR (Updated - eventLocation) ====================
window.showBookingReceipt = function (booking) {
  const content = `
        ...
        <div class="p-6 text-center">
            <div class="flex justify-center mb-4">
                <div id="receiptQRCode" class="qr-code-large"></div>
            </div>
            ...
            <div class="text-left space-y-3 bg-gray-50 rounded-xl p-4">
                ...
                <div class="flex justify-between text-sm"><span class="text-gray-500">Tempat Kegiatan</span><span class="font-medium text-gray-800">${escapeHtml(booking.eventLocation) || "-"}</span></div>
                ...
                <div class="pt-2 border-t border-gray-200"><p class="text-xs text-gray-500 mb-1">Keperluan</p><p class="text-sm text-gray-700">${escapeHtml(booking.purpose)}</p></div>
                ${booking.notes ? `<div class="pt-1"><p class="text-xs text-gray-500 mb-1">Catatan</p><p class="text-sm text-gray-700">${escapeHtml(booking.notes)}</p></div>` : ""}
            </div>
        </div>
  ...
  setTimeout(() => {
    generateQRCodeLarge("#receiptQRCode", booking.id);
  }, 100);
};
