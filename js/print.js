...
// ==================== PRINT BOOKING WITH QR CODE ====================
// ==================== PRINT BOOKING WITH QR CODE (ASYNC + ERROR HANDLING) ====================
window.printBooking = async function(id) {
  try {
    // ✅ Await async function getBookings()
    const bookings = await getBookings();
    
    // ✅ Check if bookings is array before using .find()
    const booking = Array.isArray(bookings) ? bookings.find(b => b.id === id) : null;
    
    if (!booking) {
      showToast('Data peminjaman tidak ditemukan', 'error');
      console.warn('Booking not found:', id, 'Available bookings:', bookings?.length || 0);
      return;
    }
    ...
    // showToast('Memulai proses cetak...', 'info');
