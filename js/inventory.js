...
// ==================== RENDER INVENTORY GRID (With Loading Overlay) ====================
window.renderInventory = async function () {
  try {
    ...
    showLoading('📦 Memuat data inventaris...');
    ...
    if (!equipment || equipment.length === 0) {
      ...
      container.innerHTML = `
        <div class="col-span-full text-center py-12">
          ...
          <button onclick="showAddEquipment()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700 transition">
            + Tambah Peralatan
          </button>
        </div>
      `;
    ...
    console.log('🎨 Rendering inventory grid...');
      ...
      .map((eq) => {
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 card-hover border border-gray-100">
          <div class="flex items-start justify-between mb-3">
            <div>
              <p class="font-mono text-xs text-indigo-600 font-semibold">${escapeHtml(eq.id)}</p>
              <h4 class="font-semibold text-gray-800 mt-1">${escapeHtml(eq.name)}</h4>
              <p class="text-xs text-gray-400">${escapeHtml(eq.category)}</p>
            </div>
            <span class="text-xs px-2 py-1 rounded-full font-medium ${eq.condition === "Baik" ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : "bg-red-100 text-red-700 border border-red-200"}">
              ${eq.condition}
            </span>
          </div>
          ...
          <div class="mb-3">
            <div class="flex justify-between text-sm mb-1">
              <span class="text-gray-500 font-medium">Tersedia</span>
              <span class="font-bold text-gray-800">${eq.available} / ${eq.qty}</span>
            </div>
          ...
          <div class="flex gap-2 pt-2 border-t border-gray-100">
            <button onclick="editEquipment('${eq.id}')" class="flex-1 text-sm px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600 transition font-medium">
              ✏️ Edit
            </button>
            <button onclick="deleteEquipment('${eq.id}')" class="text-sm px-3 py-2 border border-red-200 rounded-lg hover:bg-red-50 text-red-600 transition font-medium">
              🗑️ Hapus
            </button>
          </div>
        </div>
    ...
    console.log('✅ Inventory grid rendered successfully');
    ...
    if (container) {
      container.innerHTML = `
        <div class="col-span-full text-center py-12">
          ...
          <button onclick="renderInventory()" class="px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700 transition inline-flex items-center gap-2">
...
window.editEquipment = async function (id) {
  ...
  try {
    ...
    const newQtyInput = prompt("Jumlah total (saat ini: " + eq.qty + "):", eq.qty);
    ...
    const newCondition = prompt("Kondisi (Baik/Rusak):", eq.condition) || eq.condition;
    const newAvailable = parseInt(prompt("Jumlah tersedia (saat ini: " + eq.available + "):", eq.available)) || eq.available;
...
window.deleteEquipment = async function (id) {
  ...
  if (!confirm("Hapus peralatan ini secara permanen?\n\nTindakan ini tidak dapat dibatalkan.")) {
    return;
  }
